...
The RMT SHALL find requirements that it manages in the controlled git repository inside `.md` files.
...
The requirement IDs in the generated reports SHALL link to the requirement definition.
...
The RMT SHALL enforce that requirement numbers are allocated without repetition or gaps.
...
##### REQ-TRAQ-SWH-21 Requirement deletion

The RMT SHALL allow requirements marked as deleted to not have a description.

###### Attributes:
- Rationale: Deleting requirements entirely would create gaps in the IDs succession.
- Parents: REQ-TRAQ-SYS-5
- Derived: Yes
- Verification: Unit test
- Safety impact: None
...
references, ordered from system to high level to low level requirement to
implementation to test cases and procedures to verification reports.
...
references, ordered from verification reports to test cases and procedures to
implementation to low level to high level to system requirement.
...
##### REQ-TRAQ-SWH-6 Version differential

The RMT SHALL be able to generate a list of requirements at a specified version which have been added to or changed from a previously checked in version, wherein the updated material is traced bidirectionally.
...
##### REQ-TRAQ-SWH-23 Implementation association with LLR(s) or HLR(s)
...
The requirement sets of all the above outputs SHALL be filterable by ID, title or body.
...
The RMT SHALL be able to verify whether the appropriate requirement-level attribute fields are correctly completed and whether the mandatory ones are present.
...
##### REQ-TRAQ-SWH-22 Safety impact inheritance
